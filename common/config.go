...
import (
	"fmt"
	"github.com/btceasypay/bitcoinpay-miner/common/go-flags"
...
const (
	defaultConfigFilename = "bitcoinpay.conf"
)
...
type OptionalConfig struct {
	...
	UseDevices          string `long:"use_devices" description:"all gpu devices,you can use ./bitcoinpay-miner -l to see. examples:0,1 use the #0 device and #1 device"`
	...
	TaskForceStop       bool   `long:"task_force_stop" description:"force stop the current task when miner fail to get blocktemplate from the bitcoinpay full node." default-mask:"true"`
...
func CheckBase58Addr(addr, network string, p *params.Params) bool {
	...
	if err != nil {
		log.Fatalln(network, "bitcoinpay address error!", err, addr)
		return false
	}
	networkChar := addr[0:1]
	if p.NetworkAddressPrefix != networkChar {
		log.Fatalln(network, "bitcoinpay address not match the network,please check your config network param!", p.NetworkAddressPrefix, networkChar)
		return false
	}
	return true
}
...
func InitNet(network string, p *params.Params) *params.Params {
	...
	default:
		log.Fatalln(network, "Please define the network parameter for bitcoinpay!")
		return nil
	}
	return p
}
