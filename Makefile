EXECUTABLE := bitcoinpay-miner
GITVERSION := $(shell git rev-parse --short HEAD)
DEV=dev
RELEASE=release
LDFLAG_DEV = -X github.com/btceasypay/github.com/btceasypay/bitcoinpay-miner/version.Build=$(DEV)-$(GITVERSION)
LDFLAG_RELEASE = -X github.com/btceasypay/github.com/btceasypay/bitcoinpay-miner/version.Build=$(RELEASE)-$(GITVERSION)
...
VERSION=$(shell ./build/bin/bitcoinpay-miner --version | grep ^bitcoinpay-miner | cut -d' ' -f3|cut -d'+' -f1)
GOBIN = ./build/bin
...
.PHONY: bitcoinpay-miner release

bitcoinpay-miner: miner-build
	@echo "Done building."
	@echo "  $(shell $(GOBIN)/bitcoinpay-miner --version))"
	@echo "Run \"$(GOBIN)/bitcoinpay-miner\" to launch."

miner-build:
	@go build -o $(GOBIN)/bitcoinpay-miner  -tags $(TAGS) $(GOFLAGS_DEV) "github.com/btceasypay/bitcoinpay-miner"
checkversion: miner-build
#	@echo version $(VERSION)

all: bitcoinpay-miner
...
build/release/%/$(EXECUTABLE):
	@echo Build $(@)
	go build $(GOFLAGS_RELEASE) -o $(@) -tags $(TAGS) "github.com/btceasypay/bitcoinpay-miner"
...
clean:
	@rm -f *.zip
	@rm -f *.tar.gz
	@rm -f ./build/bin/bitcoinpay-miner
	@rm -rf ./build/release
