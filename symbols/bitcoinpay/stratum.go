...
func (s *BitcoinpayStratum) handleSubscribeReply(resp interface{}) {
...
func (s *BitcoinpayStratum) handleStratumMsg(resp interface{}) {
	...
	case "client.get_version":
		...
		msg := StratumMsg{
			Method: nResp.Method,
			ID:     nResp.ID,
			Params: []string{"github.com/btceasypay/bitcoinpay-miner/v0.0.1"},
		}
...
func (s *BitcoinpayStratum) handleNotifyRes(resp interface{}) {
	s.Lock()
	defer s.Unlock()
...
func (s *NotifyWork) PrepBitcoinpayWork() []byte {
	...
	for _, h := range s.MerkleBranches {
		...
		merkle_root = string(bitcoinpay.DoubleHashB([]byte(bs)))
	}
...
func (s *NotifyWork) PrepWork() error {
	...
	s.WorkData = s.PrepBitcoinpayWork()
	if s.WorkData == nil {
